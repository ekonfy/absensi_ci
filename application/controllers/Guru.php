...
class Guru extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if (!$this->input->get('kelas') && !$this->input->get('jurusan')) {
            $siswa = $this->M_guru->datasiswa();
            // $data = [
            //     'title' => WEBNAME . 'Data Siswa',
            //     'webname' => WEBNAME,
            //     'kelasjurusan' => $this->M_guru->DataSiswaByKelasDanJurusan(),
            //     'user' =>  $this->M_user->getUserById($this->session->userdata('id'))[0]
            // ];
            // $this->load->view('templates/header', $data);
            // $this->load->view('guru/kelasjurusan');
            // $this->load->view('templates/footer');
        } else {
            $idkelas = $this->input->get('kelas');
            $siswa = $this->M_guru->datasiswaByKelas($idkelas);
        }
    ...
    public function add()
    {
        ...
        $this->load->view('templates/header', $data);
        $this->load->view('guru/formtambahsiswa');
        $this->load->view('templates/footer');
    }
    // import
 ...
 //   ]);
    ...
    if ($this->form_validation->run() != FALSE) {
        if ($this->input->post('jeniskelamin')) {
            ...
            $data = [
                ...
                'kode_jurusan' => 'gtk', // Jurusan yang baru ditambahkan
                'gambar' => 'default.jpg',
				'type' => '1'
            ];
            $this->M_guru->inputsiswa($data);
    ...
    public function edit()
    {
        if ($this->input->post('nis') != FALSE) {
            $nis = $this->input->post('nis');
            $datasiswa = $this->M_guru->dataspesifiksiswa($nis)[0];
            $data = [
            ...
            $this->load->view('templates/header', $data);
            $this->load->view('guru/formeditsiswa');
            $this->load->view('templates/footer');
        } else {
            redirect(base_url() . 'siswa');
        }
    ...
    public function delete()
    {
        if ($this->input->post('id_siswa') != FALSE) {
            $this->M_guru->deletesiswa($this->input->post('id_siswa'));
	...
	public function tambahkelas()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data = [
				...
				'type' => '1' // Menambahkan kolom type dengan nilai string '1'
			];
			
			// Panggil fungsi inputkelas di model dengan data baru
			$this->M_kelas->inputstatus($data);
