...
<body>
            ...
            <div class="col-lg-8 col-sm-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <b>Scan RFID below:</b> <!-- Ganti label -->
                    </div>
                    <div class="card-body" id="bodybody">
                        <!-- Ganti placeholder dan label untuk RFID -->
                        <input type="text" id="rfidInput" class="form-control" placeholder="Scan RFID here" autofocus>

                        <!-- PHP logic untuk menampilkan waktu absensi tetap sama -->
                        <?php
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            // Input RFID menggunakan keypress event
            $('#rfidInput').on('keypress', function(e) {
                if (e.which === 13) {
                    let rfid = $(this).val(); // Ambil input dari RFID
                    setResult(rfid); // Kirim data NIS ke setResult
                    $(this).val(''); // Bersihkan input setelah scan
                }
            });
        });

        function setResult(rfid) {
            $.ajax({
                type: 'POST',
                url: "<?php echo base_url(); ?>/ajax/inputabsen",
                data: { nis: rfid }, // Kirim data NIS yang di-scan
                dataType: 'json',
